...
void	exec(char **cmd_args, char **envp)
{
	char	*cmd_with_path;
	// char	**cmd_args;
...
t_deque	*parsing_pipe(char **arg)
{
	...
	while (arg[i])
	{
		...
		{
			// node->infile = subset_arr(arg, i + 1, i + 1);
		...
		// else if (ft_strcmp(arg[i], ">") == 0)
		// {
		// 	node->outfile_ow = subset_arr(arg, i + 1, i + 1);
		// 	i += 2;
		// 	// continue ;
		// }
		// else if (ft_strcmp(arg[i], "<<") == 0)
		// {
		// 	node->here_doc = subset_arr(arg, i + 1, i + 1);
		// 	i += 2;
		// 	// continue ;
		// }
		// else if (ft_strcmp(arg[i], ">>") == 0)
		// {
		// 	node->outfile_apd = subset_arr(arg, i + 1, i + 1);
		// 	i += 2;
		// 	// continue ;
		// }
		// else if (ft_strcmp(arg[i], "|") == 0)
		...
		else	//커맨드인 경우
		{
			...
			// while (1)
			// {
			// 	if (check_redir(arg[end_i]) != 'e')
			// 		break ;
			// 	if (arg[end_i] == NULL)
			// 		break ;
			// 	end_i++;
			// }
			// node->cmd = subset_arr(arg, i, end_i - 1);
			// i = end_i;
		}
		// i++;


		// flag = check_redir(arg[i]);
		// if (flag == 'i')
		// {
		// 	node->infile = subset_arr(arg, i + 1, i + 1);
		// }
...
//----------------------------------------------------
	...
	// char	*argum = "< infile0 grep 1 | grep a | wc -l > outfile";
	// char	*argum = "< infile0 grep r | grep 2 | grep c";
	char	*argum = "< infile0 grep r | grep 2";
	char	**arg_split = ft_split(argum, ' ');
//----------------------------------------------------
...
//----------------------------------------------------
	...
	pid = malloc(sizeof(pid_t) * (information.num_pipe + 1));
	while (node)
	{
		pid[node->idx] = fork();
		if (pid[node->idx] < 0)
			ft_p_error("Error: fork()");
		else if (pid[node->idx] == 0)
		{
			// printf("%d\n", node->idx);
			if (node->idx > 0)
				waitpid(pid[node->idx - 1], NULL, 0);
			if (node->prev)
			{
				// printf("test%d\n", node->idx);
	...
	// for (int i=0; i<information.num_pipe; i++)
	// 	wait(NULL);
...
*/
	...
	// char	**sub_arr;
	// sub_arr = subset_arr(arg_split, 2, 5);
	// for (int i=0; sub_arr[i]; i++)
	// {
	// 	printf("%s, ", sub_arr[i]);
	// }
	// printf("\n\n");

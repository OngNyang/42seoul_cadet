...
/*   Updated: 2023/01/20 12:19:10 by hyejeong         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*ft_get_post_str(char *stt_buffer)
{
	...
	str = (char *)malloc(sizeof(char) * (ft_strlen(stt_buffer) - i + 1));
	if (!str)
	{
		free(stt_buffer);
		return (NULL);
...
static char	*ft_strmerge(char *stt_buffer, char *buffer)
{
	...
	str = ft_strjoin(stt_buffer, buffer);
	if (!str)
	{
		return (ft_read_err(&buffer, &stt_buffer));
	}

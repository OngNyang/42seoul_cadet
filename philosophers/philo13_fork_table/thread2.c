...
/*   Updated: 2023/05/20 01:28:29 by hyejeong         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	grab_fork(t_philo *philo)
{
	...
	while (1)
	{
		pthread_mutex_lock(&(philo->simul->arr_forks[philo->r_fork].mutex_status));
		if (philo->simul->arr_forks[philo->r_fork].status == 0)
		{
			pthread_mutex_lock(&(philo->simul->arr_forks[philo->r_fork].mutex_fork));
			mutex_printf(philo->simul, "has taken a fork", philo->id);
			philo->simul->arr_forks[philo->r_fork].status = 1;
			pthread_mutex_unlock(&(philo->simul->arr_forks[philo->r_fork].mutex_status));
			break ;
		}
		pthread_mutex_unlock(&(philo->simul->arr_forks[philo->r_fork].mutex_status));
	}
...
{
	pthread_mutex_lock(&(philo->simul->arr_forks[philo->l_fork].mutex_status));
	philo->simul->arr_forks[philo->l_fork].status = 0;
	pthread_mutex_unlock(&(philo->simul->arr_forks[philo->l_fork].mutex_status));
	pthread_mutex_unlock(&(philo->simul->arr_forks[philo->l_fork].mutex_fork));
	pthread_mutex_lock(&(philo->simul->arr_forks[philo->r_fork].mutex_status));
	philo->simul->arr_forks[philo->r_fork].status = 0;
	pthread_mutex_unlock(&(philo->simul->arr_forks[philo->r_fork].mutex_status));
